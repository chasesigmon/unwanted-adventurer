...
import {
  ...
  ROAD_TO_KORTHO_GRIMOAK_SIGN_POSITION,
...
export class WorldScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    this.load.image(SHOP_COUNTER_TEXTURE_KEY, '/shop-counter.png');
  ...
  private renderMap(mapName: MapName): void {
    ...
    // their own exterior). A later follow-up ask ("update the shop
    // sprites in Floro to look like the ones you placed in Kortho with
    // the names of them above") replaced Floro's own distinct timber-
    // shopfront art with the SAME stone-medieval KORTHO_SHOP_TEXTURE_KEY
    // spritesheet Kortho uses — FLORO_SHOP_MAPS and KORTHO_SHOP_MAPS share
    // the exact same 7 shop names in the exact same order, so the one
    // spritesheet (with its own baked-in name banners) works unmodified
    // for both towns, same "one frame per shop, indexOf into its own
    // SHOP_MAPS" lookup Kortho already uses below (no more left/right
    // mirroring — every door in the new two-row grid layout faces the
    // same way). Anchored directly ON the door's own MapExit tile — the
    // building's own baked-in door art (touching the frame's bottom edge)
    // IS the door, no separate sprite (see floroShopDoorExits' own
    // `kind: 'open'`).
      ...
      mapName === 'Floro'
        ? def.exits
            ...
            .map((exit) => {
              ...
              const frame = (FLORO_SHOP_MAPS as readonly string[]).indexOf(exit.toMap);
              return this.add
  ...
  applyMapState(state: MapStatePayload): void {
    ...
    for (const v of state.vendors) {
      ...
      // The shopfront stall (or, for Floro/Kortho, a real counter — a
      // later follow-up ask: "put the shop keeper behind a real desk
      // instead of a shop front... make the desks wider, but not as
      // tall") sits directly in front of (one tile south of) the
      // shopkeeper, who stands behind it. Bramwick's shops are
      // unaffected, keeping the original stall.
      const frontPos = this.tilePosition(v.row + 1, v.col);
      const usesCounter = (FLORO_SHOP_MAPS as readonly string[]).includes(v.map) || (KORTHO_SHOP_MAPS as readonly string[]).includes(v.map);
      const frontSprite = usesCounter
        ? this.add.sprite(frontPos.x, frontPos.y, SHOP_COUNTER_TEXTURE_KEY).setOrigin(0.5, 0.85).setDepth(-0.5)
      ...
      const sprite = this.add
        ...
        .setScale(CHAR_SCALE)
        // A follow-up bug fix: "shopkeepers are standing on top of the
        // desk. They should be behind the desk" — the counter/desk prop
        // above sits at depth -0.5, but every OTHER character sprite in
        // this scene uses Phaser's default depth (0), which always wins
        // regardless of row — the vendor rendered in front of its own
        // counter no matter how they were positioned. A vendor never
        // moves, so a plain fixed depth below the counter's own is enough
        // (unlike a walking player, who has to render in front of props
        // it can walk past).
        .setDepth(usesCounter ? -0.6 : 0)
        .setInteractive({ useHandCursor: true });
