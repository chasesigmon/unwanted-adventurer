...
// pixel-mcp available in this environment). The castle is a single tall
// image (two spired towers flanking a crenellated keep, lit windows, a
// glowing archway); flight/flicker motion comes from Phaser tweens
// applied to these otherwise-static textures, not frame animation — the
// same treatment this project's wall torches already use.
export const CASTLE_EXTERIOR_TEXTURE_KEY = 'castle-exterior';
export const CASTLE_EXTERIOR_WIDTH = 384;
export const CASTLE_EXTERIOR_HEIGHT = 672;
// Rendered 5x its raw pixel size (item 2) via a plain Phaser setScale —
// simpler and just as crisp (pixelArt:true keeps scaling nearest-
// neighbor) as regenerating the source art at higher resolution would've
// been, and shared/maps.ts's own castle-collision footprint (see
// shared/lighting.ts's isCastleExteriorBlocked) is expressed in these
// same already-scaled tile dimensions.
export const CASTLE_EXTERIOR_SCALE = 5;
export const CROW_TEXTURE_KEY = 'crow';
export const FIREPLACE_TEXTURE_KEY = 'fireplace';
