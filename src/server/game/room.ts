...
// Placeholder content — just the map name "for now". This is the seam
// where a real authored per-room name would replace the formula without
// changing RoomInfo's shape.
export function getRoomName(mapName: MapName): string {
  return mapName;
}
...
export function resolveRoom(location: Location): RoomInfo {
  return {
    ...
    name: getRoomName(location.mapName),
