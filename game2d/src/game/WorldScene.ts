...
import { MONSTER_KINDS, FLORO_SHOP_MAPS } from '../../shared/constants.js';
...
import { logChatMessage, logCombatMessage, noteCombatActivity, openChatInputWithText, setLogTabVisible } from '../ui/log.js';
...
export class WorldScene extends Phaser.Scene {
  ...
  // Own hp/mana/movement bars are a 3-bar stack (show all three above the
  // player, not just hp) — other players/NPCs/monsters keep the single
  // hp-only bar, since NpcSnapshot/MonsterSnapshot don't carry mana/
  // movement at all. Public (not private) so module-level handlers that
  ...
  private attemptMove(direction: Direction): void {
    ...
    this.network
      ...
      .then((ack) => {
        ...
        // Every successful step costs movement points — keep the status
        // bar's MV readout (and its own bar fill) in sync rather than
        // only patching `map` on a transition below.
        if (myProfile) setMyProfile({ ...myProfile, movement: ack.player.movement, maxMovement: ack.player.maxMovement });
        updateStatusBar();
        this.updateOwnBars();
