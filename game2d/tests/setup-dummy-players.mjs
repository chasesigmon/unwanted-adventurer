...
export const DUMMY_PLAYERS = [
  { name: 'Skarn', level: 8, specialization: null, map: 'Grimoak Grounds', row: 8, col: 12 },
  { name: 'Voltris', level: 9, specialization: null, map: 'Grimoak Grounds', row: 15, col: 30 },
  { name: 'Grimhale', level: 12, specialization: 'shaman', map: 'Grimoak Grounds', row: 20, col: 50 },
  { name: 'Ashcroft', level: 13, specialization: 'elementalist', map: 'Grimoak Grounds', row: 10, col: 70 },
  { name: 'Nightbrook', level: 14, specialization: 'summoner', map: 'Grimoak Grounds', row: 18, col: 90 },
  { name: 'Thornquist', level: 15, specialization: 'illusionist', map: 'Grimoak Grounds', row: 70, col: 15 },
  { name: 'Ravenmoor', level: 16, specialization: 'battlemage', map: 'Grimoak Grounds', row: 65, col: 35 },
  { name: 'Emberlyn', level: 18, specialization: 'cleric', map: 'Grimoak Grounds', row: 75, col: 55 },
  { name: 'Duskrider', level: 19, specialization: 'druid', map: 'Grimoak Grounds', row: 68, col: 75 },
  { name: 'Frostwyn', level: 20, specialization: 'diabolist', map: 'Grimoak Grounds', row: 72, col: 92 },
  // Item 8's own follow-up ask: "add a dummy player level 20 inside of
  // Grimoak Castle in the middle of the benches on the 1st floor" — the
  // Entrance Hall's own 4 benches surround (midRow, midCol) at a 3-tile
  // offset in each cardinal direction (see shared/lighting.ts's
  // benchPositionsFor), so their shared center IS that exact tile.
  { name: 'Hollowfen', level: 20, specialization: 'necromancer', map: 'Grimoak Entrance Hall', row: 17, col: 25 },
];
...
if (import.meta.url === `file://${process.argv[1]}`) {
  for (const dp of DUMMY_PLAYERS) {
    ...
    psql(
      `UPDATE players SET map='${dp.map}', "row"=${dp.row}, col=${dp.col}, level=${dp.level}, ${specClause} WHERE username='${dp.name}';`
    );
  }
