...
export class WorldScene extends Phaser.Scene {
  ...
  private renderMap(mapName: MapName): void {
    ...
    if (mapName === LUCEM_BOOK_MAP) {
      ...
      const podium = this.add
        .sprite(pos.x, pos.y, SPELLBOOK_PODIUM_TEXTURE_KEY)
        .setOrigin(0.5, 0.85)
        .setDepth(-0.5)
  ...
  useTargetedSkill(skillName: string): void {
    ...
    // Lucem is a no-target toggle too (item 11: "the player would simply
    // click on it to either create light on the wand or to remove
